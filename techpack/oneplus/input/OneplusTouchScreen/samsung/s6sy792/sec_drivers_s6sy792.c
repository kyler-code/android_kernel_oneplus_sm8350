...
static int sec_enable_edge_limit(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
...
static int sec_enable_wireless_charge_mode(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
...
static int sec_enable_reverse_wireless_charge(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
...
static int sec_audio_noise_mode(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
...
static int sec_enable_face_mode(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
...
static int sec_face_reduce_mode(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
...
static int sec_touchhold_switch_mode(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
	int i = 0;
...
static int sec_toucharea_switch_mode(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
...
static int sec_limit_switch_mode(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;
...
static int sec_gesture_switch_mode(struct chip_data_s6sy792 *chip_info, bool enable)
{
	int ret = -1;

	if (enable) {
		ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_DISABLE_GESTURE_MODE, 1); //disable gesture
		ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_SET_POWER_MODE, 0x04);//enter sleep mode
	} else {
		ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_DISABLE_GESTURE_MODE, 0); //enable gesture
		ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_SET_POWER_MODE, 0x01);//enter lp mode
	}
...
static int sec_send_temperature(void *chip_data, int temp, bool status)
{
	...
	if (status) {
		ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_NORMAL_TEMPERATURE, temp);
		TPD_INFO("temperature is %d\n", temp);
	} else {
		ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_NORMAL_TEMPERATURE, 0x7F);
		TPD_INFO("can;t get the device temperature\n");
	}

	return ret;
}

static int sec_report_rate_switch_mode(void *chip_data, int value)
{
	int ret = -1;
	struct chip_data_s6sy792 *chip_info = (struct chip_data_s6sy792 *)chip_data;

	if (value == 1) {
		ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_REFRESH_RATE_SWITCH, 0xF0);
	} else {
		ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_REFRESH_RATE_SWITCH, 0x78);
	}
	TPD_INFO("%s: report_rate_switch: %s %s!\n", __func__, value == 0 ? "120HZ":"240HZ", ret < 0 ? "failed" : "success");
	return ret;
}
...
int sec_wait_for_ready(struct chip_data_s6sy792 *chip_info, unsigned int ack)
{
	int rc = -1;
...
static int sec_limited_flash_page_write(struct chip_data_s6sy792 *chip_info, u32 page_idx, u8 *page_data)
{
	int ret = -1;
	u8 *tCmd = NULL;
...
int sec_read_calibration_report(struct chip_data_s6sy792 *chip_info)
{
	...
	ret = touch_i2c_read(chip_info->client, &buf[0], 1, &buf[1], 8);
	if (ret < 0) {
		TPD_INFO("%s: failed to read, ret = %d\n", __func__, ret);
		return ret;
	}

	TPD_INFO("%s: count:%d, pass count:%d, fail count:%d, status:0x%X, version1:0x%X, version2: 0x%X\n",
			__func__, buf[1], buf[2], buf[3], buf[4], buf[7], buf[8]);

	return buf[4];
}

int sec_execute_force_calibration(struct chip_data_s6sy792 *chip_info)
{
	int rc = -1;
...
static void check_sync_signal(void *chip_data)
{
	int ret = 0;
	struct chip_data_s6sy792 *chip_info = (struct chip_data_s6sy792 *)chip_data;

	ret = touch_i2c_write_byte(chip_info->client, SEC_SYNC_SIGNAL_STATUS, 0x01);
	ret = touch_i2c_read_byte(chip_info->client, SEC_SYNC_SIGNAL_STATUS);
	TPD_INFO("signal status is 0x%X\n", ret);
	if (ret & 0xFF)
		TPD_INFO("get the sync signal from display\n");
	else
		TPD_INFO("no sync signal from display\n");
}
static void sec_get_ic_firmware_version(void *chip_data, struct panel_info *panel_data)
{
...
static struct touchpanel_operations sec_ops = {
	...
	.switch_report_rate			= sec_report_rate_switch_mode,
	.get_ic_firmware_version	= sec_get_ic_firmware_version,
};
/********* End of implementation of touchpanel_operations callbacks**********************/


/**************** Start of implementation of debug_info proc callbacks************************/
int sec_fix_tmode(struct chip_data_s6sy792 *chip_info, u8 mode, u8 state)
{
	int ret = -1;
...
int sec_release_tmode(struct chip_data_s6sy792 *chip_info)
{
	int ret = -1;
...
static int sec_execute_selftest(struct seq_file *s, struct chip_data_s6sy792 *chip_info, struct sec_testdata *sec_testdata)
{
	int rc = -1;
...
int sec_execute_p2ptest(struct seq_file *s, struct chip_data_s6sy792 *chip_info, struct sec_testdata *sec_testdata)
{
	int rc;
...
static void sec_auto_test(struct seq_file *s, void *chip_data, struct sec_testdata *sec_testdata)
{
	...
	if (sec_testdata->test_item & (1 << TYPE_MUTUAL_RAW_OFFSET_DATA_SDC)) {
		...
		tp_test_write(sec_testdata->fd, sec_testdata->length, data_buf, strlen(data_buf), sec_testdata->pos);
		sprintf(data_buf, "RX-TX, ");
		tp_test_write(sec_testdata->fd, sec_testdata->length, data_buf, strlen(data_buf), sec_testdata->pos);
		for(i = 0; i < sec_testdata->TX_NUM; i++) {
			sprintf(data_buf, "TX%d, ", i);
			tp_test_write(sec_testdata->fd, sec_testdata->length, data_buf, strlen(data_buf), sec_testdata->pos);
		}
		sprintf(data_buf, "\n");
		tp_test_write(sec_testdata->fd, sec_testdata->length, data_buf, strlen(data_buf), sec_testdata->pos);
		for (j = 0; j <  sec_testdata->RX_NUM; j++) {
			sprintf(data_buf, "RX%d:, ", j);
			tp_test_write(sec_testdata->fd, sec_testdata->length, data_buf, strlen(data_buf), sec_testdata->pos);
			for (i = 0; i <  sec_testdata->TX_NUM; i++) {
			...
			if (!IS_ERR_OR_NULL(sec_testdata->fd)) {
				sprintf(data_buf, "\n");
				tp_test_write(sec_testdata->fd, sec_testdata->length, data_buf, strlen(data_buf), sec_testdata->pos);
			}
		}
...
int execute_selftest(struct chip_data_s6sy792 *chip_info, bool save_result)
{
	u8 pStr[50] = {0};
	u8 pTmp[20];
	int rc = 0;
...
static void sec_ts_print_channel(struct chip_data_s6sy792 *chip_info)
{
...
static int sec_ts_read_channel(struct chip_data_s6sy792 *chip_info, u8 type,
...
static void sec_ts_print_frame(struct chip_data_s6sy792 *chip_info, short *min, short *max)
{
	int i = 0;
	int j = 0;
	unsigned char *pStr = NULL;
...
static int sec_ts_read_frame(struct chip_data_s6sy792 *chip_info, u8 type, short *min,
...
static void sec_apk_water_set(void *chip_data, int type)
{
	...
	if (type > 0) {
		touch_i2c_write_byte(chip_info->client, SEC_WET_MODE, 3);
	} else {
		touch_i2c_write_byte(chip_info->client, SEC_WET_MODE, 0);
	}
...
static int sec_apk_water_get(void *chip_data)
{
	...
	int ret = -1;
	chip_info = (struct chip_data_s6sy792 *)chip_data;
	ret = touch_i2c_read_byte(chip_info->client, SEC_WET_MODE);
...
static void sec_init_apk_op(struct touchpanel_data *ts)
{
...
struct i2c_driver sec_tp_i2c_driver =
{
	.probe = sec_tp_probe,
